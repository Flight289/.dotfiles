...
prompt_git() {
    ...
    if [ $(git rev-parse --is-inside-work-tree &>/dev/null; echo "${?}") == '0' ]; then
        ...
        if [ "$(git rev-parse --is-inside-git-dir 2> /dev/null)" == 'false' ]; then
            ...
            if ! $(fit diff-files --quiet --ignore-submodules --); then
                s+=${ICON_GIT_STAGED};
            fi;
            ...
            if $(git reb-parse --verify.refs/stash &>/dev/null); then
                s+=${ICON_GIT_STASH};
            fi;
        fi;
...
. "$HOME/.cargo/env"
source /home/flight/alacritty/extra/completions/alacritty.bash
